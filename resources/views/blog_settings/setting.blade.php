...
<div class="sh-breadcrumb">
    <nav class="breadcrumb">
        ...
        <a class="breadcrumb-item" href="{{ route('admin.blog.index') }}">Блог</a>
...
<div class="sh-pagebody">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.blog.setting.update') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('Заголовок страницы') }}</label>
                            <input type="text" class="form-control" name="title" value="{{ config('app.blog.title', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('Meta Title') }}</label>
                            <input type="text" class="form-control" name="meta_title" value="{{ config('app.blog.meta_title', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('Meta Description') }}</label>
                            <input type="text" class="form-control" name="meta_description" value="{{ config('app.blog.meta_description', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('Meta Keywords') }}</label>
                            <input type="text" class="form-control" name="meta_keywords" value="{{ config('app.blog.meta_keywords', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('OG Title') }}</label>
                            <input type="text" class="form-control" name="og_title" value="{{ config('app.blog.og_title', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('OG Description') }}</label>
                            <input type="text" class="form-control" name="og_description" value="{{ config('app.blog.og_description', '') }}">
                        </div>
                        <div class="form-group">
                            <label for="" class="form-control-label">{{ __('OG Image') }}</label>
                            <div class="form-group">
                                <div id="preview"style="max-width: 480px;width: 100%;">
                                    @if(config('app.blog.og_image', ''))
                                        <img src="{{ thumbnail(config('app.blog.og_image', ''), 480) }}" class="img-thumbnail" alt="Preview">
                                    @endif
                                    <input type="hidden" name="image" value="{{ config('app.blog.og_image', '') }}">
